...
class GoRec_Learner(nn.Module):
    ...
    def _init_model(self):
        ...
        item_emb_mapped = torch.empty(*item_emb.shape)
    ...
    def clustering(self):
        ...
        cluster_cfmean = []
        for i in range(self.args.pre_cluster_num+1):
...
class Decoder(nn.Module):
    def __init__(self, z_size, latent_dim, layer, si_dim):
        ...
        for i in range(layer):
            layers.append(EncoderBlock(input_dim=self.size, output_dim=self.size))
            self.size = latent_dim
