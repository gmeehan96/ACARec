...
def main():
    def objective(config):
        ...
        model = eval(inp_args.model)(inp_args, training_data, warm_valid_data, cold_valid_data, all_valid_data,
                                            ...
                                            user_content=None, item_content=item_content)
        model.train()
    ...
    parser.add_argument('--feat_dir', type=str, default='feats', help='Feat file location')
    ...
    parser.add_argument('--artist_mean_embs_file',type=str,help='pt file containing artist means')
    ...
    if args.model == 'CLCRec':
        configs = {'m4a_onion':{'temp_value': 1.0,
                                    'lr_lambda': 0.2,
                                    'num_sample': 0.9,
                                    'lr':0.001,
                                    'reg':0.0001},
                        'yambda':{'temp_value': 1.0,
                                    'lr_lambda': 0.6,
                                    'num_sample': 0.6,
                                    'lr':0.001,
                                    'reg':0.0001}
        ...
        configs = {'m4a_onion':{'False':{'alpha': 0.001,
                                            'n_dropout': 0.5,
                                            'n_expert': 7,
                                            'lr':0.0001,
                                            'reg':0.00001,
        ...
        configs = {'m4a_onion':{'False':{'alpha': 1.0,
                                            'beta': 0.0,
                                            'lr':0.0001,
                                            'reg':0.000001,
                                            'use_artist_mean':False},
        ...
        args.bs = 1024
        args.patience = 10
        config = {'emb_size':128 if args.dataset == 'yambda' else 512,
                            }
   
        config = {**config,**configs[dataset][artist_mean]}
    elif args.model == 'DeepMusic':
        configs = {'m4a_onion':{'False':{'use_artist_mean':False,
                                            'lr':0.001,
                                            'reg':0.0},
                                    'True': {'use_artist_mean':True,
                                            'lr':0.001,
                                            'reg':0.000001}  },
                        'yambda':{'False':{'use_artist_mean':False,
                                            'lr':0.0003,
                                            'reg':0.0001},
                                    'True': {'use_artist_mean':True,
                                            'lr':0.0003,
